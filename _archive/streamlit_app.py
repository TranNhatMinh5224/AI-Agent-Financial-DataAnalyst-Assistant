...
from financial_text_to_pandas.workspace import get_workspaces, create_workspace, get_workspace_artifacts_path, get_workspace_uploads_path
import subprocess
...
try:
    cfg, orchestrator = init_system(
        ...
        planner_model=os.getenv("LLM_PLANNER_MODEL", "deepseek-r1:14b"),
        retriever_model=os.getenv("LLM_RETRIEVER_MODEL", "qwen2.5:7b"),
        programmer_model=os.getenv("LLM_PROGRAMMER_MODEL", "qwen2.5-coder:14b"),
        critic_model=os.getenv("LLM_CRITIC_MODEL", "qwen2.5-coder:3b")
    )
...
with st.sidebar:
    st.header("🗂️ Quản lý Dự án (Workspaces)")
    workspaces = get_workspaces()
    if not workspaces:
        st.warning("Chưa có dự án nào. (Kể cả bộ ViFinQA mặc định cũng không tìm thấy!)")
        
    ws_options = {ws["id"]: f"{ws['name']} ({ws['id']})" for ws in workspaces}
    selected_ws_id = None
    if workspaces:
    ...
    new_ws_name = st.text_input("Tên dự án mới:")
    if st.button("Tạo & Chuyển sang dự án mới", use_container_width=True):
        if new_ws_name:
            new_id, _ = create_workspace(new_ws_name)
            st.success(f"Đã tạo {new_id}!")
            st.rerun()
            
    if selected_ws_id and selected_ws_id != "vifinqa_core":
        st.divider()
        st.subheader("📤 Upload Dữ Liệu")
        uploaded_files = st.file_uploader("Upload CSV/Excel", type=["csv", "xls", "xlsx"], accept_multiple_files=True)
        if st.button("Xử lý & Embedding", type="primary", use_container_width=True):
            if uploaded_files:
                up_dir = get_workspace_uploads_path(selected_ws_id)
                for uf in uploaded_files:
                    with open(up_dir / uf.name, "wb") as f:
                        f.write(uf.getbuffer())
                
                with st.spinner("Đang chạy Pipeline (BM25, Embeddings)..."):
                    # Chạy script pipeline qua subprocess để tránh kẹt thread
                    pipe_script = Path(__file__).parent / "src" / "financial_text_to_pandas" / "preprocessing" / "generic_pipeline.py"
                    subprocess.run([sys.executable, str(pipe_script), selected_ws_id])
                st.success("Xử lý hoàn tất!")
                st.rerun()
            else:
                st.error("Vui lòng chọn file!")

# Lấy output_root cho dự án hiện tại
active_output_root = None
if selected_ws_id:
    active_output_root = get_workspace_artifacts_path(selected_ws_id)
else:
    active_output_root = cfg.output_root
...
if analyze_btn and query:
    with st.spinner("🤖 Các Agent đang hội ý phân tích dữ liệu... (khoảng 10-30s)"):
        ...
        try:
            tables = run_search(query, cfg, method="hybrid", top_k=5, project_root=active_output_root)
        except Exception as e:
            st.toast(f"Hybrid Search lỗi, tự động chuyển về BM25 ({e})", icon="⚠️")
            tables = run_search(query, cfg, method="bm25", top_k=5, no_reranker=True, project_root=active_output_root)
        ...
        # 3. Load dfs
        dfs = load_evidence_tables(package, active_output_root)
        
        # 4. Orchestrate
        trace = orchestrator.run(query, package, dfs, run_config=cfg, output_root=active_output_root)
